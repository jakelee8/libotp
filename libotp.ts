...
export class HOTP {
   ...
   *   one-time code.
   * @param {number} [params.window=2] The allowable margin for the
  ...
  public diff (token: string): number {
    ...
    // loop from C - W to C + W inclusive
 ...
 * seconds and a window size of 2, meaning a token is valid for up to
 * 1.33 minutes.
 *
 * A time period of 30 seconds with a window size of 1 results in a token
 * that is valid for up to 1.33 minutes due to time drift. For example:
 *
 * - Configuration: period=30 window=2
 * - Server: time(ms)=100 counter=`Math.floor(100/period)`=3
 * - Client: time(ms)=159 counter=`Math.floor(179/period)`=5
 * - Counter difference: `5 - 3 = 2` (valid)
 * - Time difference: `179 - 100 = 79` (~1.33 min)
 *
